...
import core.base.R;
import core.base.views.gallery.largeimage.LargeImageGallery;
import core.base.views.gallery.zoomable.ZoomableController;
import core.base.views.gallery.zoomable.ZoomableDraweeView;

/**
 * Created by boson on 2016/12/22.
 */

public class CommentGallery extends RelativeLayout
        implements LargeImageGallery.OnSelectionChangedListener,
        LargeImageGallery.OnItemClickListener,
        View.OnClickListener,
        ZoomableController.OnSwipeDownListener {
    ...
    public void onSelectionChanged(int lastIndex, int currentIndex) {
        try {
            List<String> imageUrlList = mCommentData.getImageUrl();
            imgUrl = imageUrlList.get(currentIndex);
            mTextViewIndicator.setText(String.format(INDICATOR_STRING_FORMAT, currentIndex + 1, imageUrlList.size()));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void onClick(View view) {
        int id = view.getId();
        if (id == R.id.iv_back) {
            if (mImageOperationListener != null) {
                mImageOperationListener.onBack();
            }
        } else if (id == R.id.iv_download) {
            if (mImageOperationListener != null) {
                mImageOperationListener.onDownloadImage(imgUrl);
            }
        }
    }

    OnImageOperationListener mImageOperationListener;

    public void setImageOperationListener(OnImageOperationListener operationListener) {
        this.mImageOperationListener = operationListener;
    }
